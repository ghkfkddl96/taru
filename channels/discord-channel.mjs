 ...
 * @param {string} config.memoryEnvKey — env var name for memory dir (e.g. 'PATRASCHE_MEMORY_DIR')
 * @param {string} config.memoryDefault — default memory dir name (e.g. 'Patrasche_Memory')
 */
export async function createChannel(config) {
  ...
  const MEMORY_BASE = process.env[memoryEnvKey || 'PATRASCHE_MEMORY_DIR']
    ...
    : path.join(os.homedir(), 'Documents', memoryDefault || 'Patrasche_Memory')
  ...
  const memDir = agentName.toLowerCase().replace(/[^a-z0-9]/g, '_')
  setInterval(() => {
    mcp.notification({
      method: 'notifications/claude/channel',
      params: {
        content: `[SYSTEM] 30 minutes elapsed. Write session notes to ~/Documents/${memoryDefault || 'Patrasche_Memory'}/session_notes/${memDir}/ (today's date file). Append if exists.`,
